...
7. 活动剩余天数小于一天时，C 端展示剩余小时数。
8. 剩余时长预览支持多语言。
9. 商品必须支持多 country code 和 currency 定价配置，如 `JP + JPY`、`US + USD`。
10. 当 C 端用户 IP 解析出的 country code 命中某条国家定价时，商品列表、商品详情、收银台和订单金额使用该条定价。
11. 当 C 端用户 IP 解析出的 country code 未配置时，展示默认币种和默认价格。
