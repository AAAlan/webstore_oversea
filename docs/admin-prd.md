...
| 产品名称 | 海外版充值商城管理后台 |
| 文档版本 | v1.0 |
| 文档范围 | 登录、页面内容管理、商品管理、字段级多语言配置、游戏发货配置、草稿发布、自动保存 |
...
5. 支持在页面内容、商品和商品类型配置中通过字段级弹窗维护多语言文案。
...
| 编辑 | `demo-editor-token` | 可访问内容、商品和字段级多语言编辑能力，不展示管理员专属入口 |
...
已去掉或不在当前后台展示的页面：营销活动管理、订单管理、运营工具、多语言管理。
...
```mermaid
flowchart TD
  A[进入页面内容管理或商品管理] --> B[编辑支持多语言的字段]
  B --> C[点击字段旁多语言按钮]
  C --> D[打开多语言配置弹窗]
  D --> E[填写 Key、分类、使用场景、源文案]
  E --> F[填写各语言译文]
  F --> G[保存多语言文案草稿]
  G --> H[返回当前业务配置弹窗]
```
...
| 编号 | 功能 | 说明 | 优先级 |
| --- | --- | --- | --- |
| A-PUB-01 | 进入加载规则 | 进入配置页时，若没有草稿则加载最新线上版本；若存在草稿则加载草稿 | P0 |
| A-PUB-02 | 草稿保存规则 | 保存草稿时使用最新线上版本的 `version`，后续持续修改并保存草稿时版本号不变 | P0 |
| A-PUB-03 | 发布版本 | 点击发布后将草稿数据同步到线上库，切换线上使用的发布版本，并删除草稿 | P0 |
| A-PUB-04 | 模块独立发布 | 页面内容、多语言、游戏发货等模块独立发布 | P0 |
| A-PUB-05 | 商品统一发布 | 商品管理页面只保留一个发布按钮，同时发布商品类型和商品列表 | P0 |
| A-PUB-06 | 草稿与线上比对 | 优先比对 `version`；若 `version` 不同则显示不一致；若 `version` 相同，则进行文本/内容比对，有草稿且内容不同也显示不一致 | P0 |
| A-PUB-07 | 草稿清理 | 发布成功后删除对应模块草稿，后续再次编辑时以最新线上版本作为新草稿基础 | P0 |

比对规则：

1. 草稿与线上版本先进行 `version` 比对。
2. `version` 不一致时，直接判定为“存在未发布变更”。
3. `version` 一致时，继续进行文本/内容比对；只要存在草稿且内容与线上版本不同，也判定为“存在未发布变更”。
4. 没有草稿且当前线上版本已是最新使用版本时，判定为“与线上一致”。
...
| 限购上限 | 0 表示不限购；不限购商品 C 端单次最多可添加 99 个 |
| 活动时效 | 活动结束时间或剩余时间展示 |
| 角标 | C 端商品卡片角标，支持多语言 |
...
5. 不支持终身限购。
6. 限购上限为 0 时表示不限购，但 C 端商品详情和下单接口仍需限制单次最多购买 99 个。
7. C 端商品详情数量选择支持加减按钮和手动输入，手动输入需按限购剩余次数、充值限额和单次 99 上限自动修正。
8. 活动剩余天数小于一天时，C 端展示剩余小时数。
9. 剩余时长预览支持多语言。
10. 商品必须支持多 country code 和 currency 定价配置，如 `JP + JPY`、`US + USD`。
11. 当 C 端用户 IP 解析出的 country code 命中某条国家定价时，商品列表、商品详情、收银台和订单金额使用该条定价。
12. 当 C 端用户 IP 解析出的 country code 未配置时，展示默认币种和默认价格。
...
字段级多语言配置不再作为左侧独立菜单展示。页面内容管理、商品管理、商品类型等业务配置中，凡是 C 端需要展示的文本字段，可通过字段旁的多语言按钮打开配置弹窗维护译文。

### 10.1 适用字段

| 字段 | 说明 |
| --- | --- |
| 页面内容文案 | 充值中心名称、充值说明文案等 |
| 商品类型名称 | 商品分类 Tab 名称 |
| 商品名称 | C 端商品卡片和详情展示 |
| 商品描述 | 商品到账说明或礼包内容 |
| 商品角标 | 商品卡片营销角标 |
...
1. 字段旁展示多语言按钮，点击后打开多语言配置弹窗。
2. 弹窗内可维护 Key、分类、使用场景、源文案和各语言译文。
3. 保存后写入多语言草稿，当前业务字段继续显示默认语言文案。
4. 左侧菜单不展示“多语言管理”，也不提供独立多语言管理页面入口。
5. 多语言发布能力由业务模块发布流程承接，正式环境可按后端发布策略统一处理。
...
1. 线上库保存已发布数据，并记录当前线上使用的发布版本 `version`。
2. 草稿库仅保存未发布编辑内容；保存草稿时写入当前最新线上版本的 `version`，后续重复保存同一草稿不递增 `version`。
3. 发布草稿时，将草稿数据同步到线上库，生成并切换新的线上发布版本，然后删除草稿。
4. 草稿删除后，后续再次进入配置页时加载最新线上版本；用户再次修改后，再以最新线上版本为基础生成新草稿。
5. 发布状态不只依赖 `version`，还需要在 `version` 相同时进行内容比对，避免“同版本但草稿文本已变更”被误判为一致。
...
| version 相同但草稿内容不同 | 判定为存在未发布变更，提示用户发布或继续编辑 |
| 发布成功后仍存在旧草稿 | 应删除旧草稿；若删除失败，需提示并阻止状态显示为已发布一致 |
