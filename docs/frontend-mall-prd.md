...
| C-DETAIL-02 | 数量选择 | 支持加减按钮和手动输入数量；根据限购剩余次数控制可选数量；不限购商品单次最多可添加 99 个 | P0 |
| C-DETAIL-03 | 总价计算 | 根据单价和数量实时计算总价 | P0 |
| C-DETAIL-04 | 立即支付 | 点击后进行限额校验，合规则打开收银台 | P0 |
| C-DETAIL-05 | 限额弹窗 | 日本未成年人累充限额场景在点击立即支付后出现提示弹窗 | P0 |
| C-DETAIL-06 | 不限购数量上限 | `limitMax = 0` 或不限购商品仍需限制单次购买数量，最大 99 个 | P0 |
| C-DETAIL-07 | 数量输入校验 | 手动输入为空、非数字、小于 1 或超过最大可购数量时，前端自动修正到合法范围 | P0 |
...
| 不限购商品超过 99 个 | 前端禁止继续增加数量，接口二次校验并提示单次最多购买 99 件 |
| 手动输入非法数量 | 失焦、变更或支付前自动修正为 1 到最大可购数量之间的整数 |
