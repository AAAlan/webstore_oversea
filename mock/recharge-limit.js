...
export const REGION_DEMO_RU_LOCKED_PRICE = "15200000009";
...
const DEMO_ACCOUNT_PROFILES = {
  ...
  [REGION_DEMO_RU_LOCKED_PRICE]: { age: 25, country: "RU" },
...
export function buildRechargeLimitView(accountId, monthlySpent) {
...
export function validateRechargeAmount(limit, amount) {
  if (!limit.canRecharge) {
    return limit.blockReason ?? "当前账号无法充值";
  }
  if (limit.singleMax != null && amount > limit.singleMax) {
    return `单次支付不能超过 ${limit.singleMax} ${limit.currencyLabel}，当前订单金额为 ${amount.toFixed(2)} ${limit.currencyLabel}`;
  }
  if (limit.monthlyRemaining != null && amount > limit.monthlyRemaining) {
    return `已超出每月 ${limit.monthlyMax.toFixed(2)} ${limit.currencyLabel}充值限额，无法支付（本月剩余额度 ${limit.monthlyRemaining.toFixed(2)} ${limit.currencyLabel}，当前订单金额 ${amount.toFixed(2)} ${limit.currencyLabel}）`;
  }
  return null;
}
...
export function getProductAgeLimitReason(limit, unitPrice, quantity = 1) {
  ...
  return validateRechargeAmount(limit, amount);
}
